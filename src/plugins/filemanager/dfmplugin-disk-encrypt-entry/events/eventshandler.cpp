...
void EventsHandler::onOverlayDMModeChanged(bool enabled, int result)
{
    ...
    if (isSuccess) {
        ...
        if (enabled) {
            title = tr("Overlay DM Mode Enabled");
            message = tr("Overlay DM mode has been enabled successfully. Please reboot your system for the changes to take effect. "
                        "After reboot, partition encryption operations will not require system restart.");
        } else {
            title = tr("Overlay DM Mode Disabled");
            message = tr("Overlay DM mode has been disabled successfully. Please reboot your system for the changes to take effect. "
                        "After reboot, partition encryption operations will require system restart.");
        }
        ...
        if (enabled) {
            message = tr("Failed to enable Overlay DM mode. The system may not support this feature or encountered an error during initramfs update.");
        } else {
            message = tr("Failed to disable Overlay DM mode. An error occurred during initramfs update.");
        }
